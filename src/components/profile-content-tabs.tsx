...
export function ProfileContentTabs({ favGames, playStatusGames, comments }: Props) {
  ...
  const loadCollections = useCallback(async () => {
    try {
      const data = await apiGet<{ collections: CollectionData[] }>("/api/collections")
  ...
  const handleCreateCollection = async () => {
    ...
    try {
      ...
      if (res.ok) { await loadCollections(); setNewFolderName(""); setShowCreateFolder(false) }
    } catch { }
  }

  const handleDeleteCollection = async (id: string) => {
    try { await fetch(`/api/collections/${id}`, { method: "DELETE" }); await loadCollections() } catch { }
  }
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="p-4 sm:p-5 profile-scroll-area">
        {active === "favorites" && (
          <FavoritesTab defaultFolderGames={defaultFolderGames} collections={collections}
            onOpenFolder={(col) => setModalCollection(col)} loading={collectionsLoading} />
        )}
      ...
      {modalCollection && createPortal(
        <FolderModal name={modalCollection.name}
...
/* ==================== 收藏夹 Tab ==================== */
function FavoritesTab({ defaultFolderGames, collections, favGames, onOpenFolder, showCreateFolder,
  setShowCreateFolder, newFolderName, setNewFolderName, onCreateFolder, onDeleteFolder, loading }: {
  defaultFolderGames: GameLite[]; collections: CollectionData[]; favGames: GameLite[]
  onOpenFolder: (col: CollectionData) => void; showCreateFolder: boolean
  setShowCreateFolder: (v: boolean) => void; newFolderName: string; setNewFolderName: (v: string) => void
  onCreateFolder: () => void; onDeleteFolder: (id: string) => void; loading: boolean
}) {
  ...
  return (
    <div className="space-y-3">
      {showCreateFolder ? (
        <div className="rounded-xl bg-secondary/50 p-4 ring-1 ring-border">
          <input type="text" value={newFolderName} onChange={(e) => setNewFolderName(e.target.value)}
            placeholder="收藏夹名称"
            className="mb-3 w-full rounded-lg bg-background px-3 py-2.5 text-sm text-foreground outline-none ring-1 ring-border focus:ring-2 focus:ring-primary/40"
          ...
          <div className="flex gap-2">
            ...
            <button onClick={onCreateFolder} disabled={!newFolderName.trim()}
              className="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground hover:bg-primary/90 disabled:opacity-40 disabled:cursor-not-allowed shadow-sm">
              创建
            </button>
          </div>
        </div>
        ...
        <button onClick={() => setShowCreateFolder(true)}
          className="flex w-full items-center justify-center gap-2 rounded-xl border-2 border-dashed border-muted-foreground/20 bg-secondary/20 px-4 py-3.5 text-sm font-medium text-muted-foreground hover:border-primary/40 hover:bg-primary/5 hover:text-primary">
      ...
      {loading ? (
        ...
        <>
          <CollectionCard id="default" name="默认收藏夹" gameCount={defaultFolderGames.length}
            ...
            isDefault />
          {collections.map((col) => (
            <CollectionCard key={col.id} id={col.id} name={col.name}
              gameCount={col.favorites?.length ?? 0} coverGames={col.favorites?.map(f => f.game) ?? []}
...
function CollectionCard({ id, name, gameCount, coverGames, onOpen, onDelete, isDefault }: {
  id: string; name: string; gameCount: number; coverGames: GameLite[]
  onOpen: () => void; onDelete?: () => void; isDefault?: boolean
}) {
  return (
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        {!isDefault && onDelete && (
          <button onClick={(e) => { e.stopPropagation(); onDelete() }}
            className="flex items-center justify-center rounded-lg p-1.5 text-muted-foreground hover:bg-rose-500/10 hover:text-rose-500"
            title="删除收藏夹">
            ...
            {coverGames.slice(0, 5).map((g) => (
              <div key={g.id} className="h-16 w-12 shrink-0 overflow-hidden rounded-md">
                {g.coverImage ? <Image src={g.coverImage} alt={g.title} width={48} height={64} className="h-full w-full object-cover" unoptimized />
                  : <div className="flex h-full w-full items-center justify-center bg-muted text-muted-foreground"><FolderHeart className="h-4 w-4" /></div>}
              </div>
            ))}
            {gameCount > 5 && (
            ...
            <div className="grid grid-cols-3 gap-3 sm:grid-cols-4 md:grid-cols-5">
              {games.map((g) => (
                <Link key={g.id} href={`/games/${g.serialId ?? g.id}`} className="group" onClick={onClose}>
                  {g.coverImage ? <Image src={g.coverImage} alt={g.title} width={120} height={160}
                    className="aspect-[3/4] w-full rounded-lg object-cover" unoptimized />
                    : <div className="flex aspect-[3/4] w-full items-center justify-center rounded-lg bg-muted text-muted-foreground"><FolderHeart className="h-6 w-6" /></div>}
...
function CommentsTab({ comments }: { comments: CommentLite[] }) {
  ...
  return (
    <div className="flex flex-col gap-2.5">
      {comments.map((c) => (
        <Link key={c.id} href={`/games/${c.game.serialId ?? c.game.id}`}
          className="group rounded-xl bg-secondary/40 p-3.5 hover:bg-secondary/70">
          <div className="flex items-center gap-2 text-[11px] text-muted-foreground mb-1.5">
            <span className="font-medium text-foreground group-hover:text-primary">{c.game.title}</span>
            <span>·</span><Calendar className="h-3 w-3" />
...
function PlayTab({ playStatusGames }: { playStatusGames: { game: GameLite; status: string }[] }) {
  ...
  if (playStatusGames.length === 0) return (
    <div className="flex flex-col items-center justify-center py-12 text-center">
      ...
      <p className="text-sm text-muted-foreground">
        {playMsg ? `${playMsg.emoji} ${playMsg.title}，${playMsg.subtitle}` : "还没有游玩记录"}
      </p>
    </div>
  )

  const colors: Record<string, string> = {
    "想玩": "bg-sky-500/10 text-sky-400", "在玩": "bg-amber-500/10 text-amber-400",
    "玩过": "bg-emerald-500/10 text-emerald-400", "搁置": "bg-muted-foreground/10 text-muted-foreground",
    "弃坑": "bg-rose-500/10 text-rose-400",
  }
  return (
    <div className="flex flex-col gap-2">
      {playStatusGames.map(({ game, status }) => (
        <Link key={game.id} href={`/games/${game.serialId ?? game.id}`}
          className="group flex items-center gap-3 rounded-xl bg-secondary/40 p-3 hover:bg-secondary/70">
          {game.coverImage ? <Image src={game.coverImage} alt={game.title} width={36} height={48} className="h-12 w-9 rounded-md object-cover" unoptimized />
            : <div className="flex h-12 w-9 items-center justify-center rounded-md bg-muted"><Gamepad2 className="h-4 w-4" /></div>}
